...
const PAYMENT_INFO = `💳 *Métodos de pago:*
• Daviplata: 3114819989
• Nequi: 3114819989
• Llave: 3114819989 o @LAT572`;
...
const initialPrices = {
  AA:    { sale: 3000, cost: 2500 },
  Extra: { sale: 3500, cost: 3000 },
  Jumbo: { sale: 4000, cost: 3400 },
};
...
function DeleteButton({ onConfirm }) {
  ...
  return (
    <button onClick={() => setConfirming(true)} className="border border-red-100 text-red-400 text-xs rounded-xl py-2 px-3 hover:bg-red-50 active:scale-95 transition-all">🗑</button>
  );
}
...
`🥚 *La Gallina Turuleca*
*Factura No. ${String(d.invoiceNum).padStart(4,"0")}* · ${formatDate(d.date)} ${new Date().getFullYear()}
...
function Resumen({ despachos, clients, prices, onDelete }) {
  const totalCobrar = despachos.reduce((s, d) => s + Math.max(0, totalDespacho(d) - d.paid), 0);
  const totalProveedor = despachos.filter(d => !d.providerPaid).reduce((s, d) => s + costoDespacho(d), 0);
          ...
          {EGG_TYPES.map(t => (
            <div key={t} className="flex justify-between items-center">
              ...
              <div className="flex gap-3 text-xs text-gray-500">
                <span>Cliente: <span className="font-semibold text-gray-800">{fmt(prices[t].sale)}</span></span>
                <span>Prov: <span className="font-semibold text-gray-800">{fmt(prices[t].cost)}</span></span>
                <span className="text-green-700 font-semibold">+{fmt(prices[t].sale - prices[t].cost)}</span>
              </div>
            </div>
          ))}
        </div>
...
function Despachar({ clients, prices, onDespacho }) {
  ...
  const [lineas, setLineas] = useState([{ tipo: "AA", cubetas: "", salePrice: prices["AA"].sale, costPrice: prices["AA"].cost }]);
  const [showSuccess, setShowSuccess] = useState(false);

  function addLinea() {
    const tiposUsados = lineas.map(l => l.tipo);
    const disponible = EGG_TYPES.find(t => !tiposUsados.includes(t));
    if (!disponible) return;
    setLineas(prev => [...prev, { tipo: disponible, cubetas: "", salePrice: prices[disponible].sale, costPrice: prices[disponible].cost }]);
  }
  ...
  function updateLinea(i, field, value) {
    setLineas(prev => prev.map((l, idx) => {
      if (idx !== i) return l;
      if (field === "tipo") {
        return { ...l, tipo: value, salePrice: prices[value].sale, costPrice: prices[value].cost };
      }
  ...
  function handleDespachar() {
    ...
    setLineas([{ tipo: "AA", cubetas: "", salePrice: prices["AA"].sale, costPrice: prices["AA"].cost }]);
    setShowSuccess(true);
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white rounded-2xl border border-gray-100 p-4 space-y-4">
        ...
        {clients.length === 0 ? (
          <p className="text-sm text-gray-400 text-center py-4">Primero agrega un cliente en la pestaña 👥</p>
        ) : (
          <>
            <div>
            ...
            <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex gap-2">
                      {EGG_TYPES.map(t => (
                        <button
                          key={t}
                          onClick={() => updateLinea(i, "tipo", t)}
                          disabled={tiposUsados.includes(t) && l.tipo !== t}
                    ...
                    {lineas.length > 1 && (
                      <button onClick={() => removeLinea(i)} className="text-red-300 hover:text-red-500 text-lg px-1">×</button>
                    )}
                  </div>
                  <div className="grid grid-cols-3 gap-2">
                    <div>
                      <label className="text-xs text-gray-400 mb-1 block">Cubetas</label>
                      <input type="number" className={inputCls} placeholder="0" min={1} value={l.cubetas} onChange={e => updateLinea(i, "cubetas", e.target.value)} />
                    </div>
                    <div>
                      <label className="text-xs text-gray-400 mb-1 block">Precio cliente</label>
                      <input type="number" className={inputCls} value={l.salePrice} onChange={e => updateLinea(i, "salePrice", e.target.value)} />
                    </div>
                    <div>
                      <label className="text-xs text-gray-400 mb-1 block">Costo prov.</label>
                      <input type="number" className={inputCls} value={l.costPrice} onChange={e => updateLinea(i, "costPrice", e.target.value)} />
                    </div>
                  </div>
                  {Number(l.cubetas) > 0 && (
              ...
              {lineas.length < EGG_TYPES.length && (
                <button onClick={addLinea} className="w-full border-2 border-dashed border-yellow-200 text-yellow-600 rounded-xl py-2 text-xs font-medium hover:bg-yellow-50 active:scale-95 transition-all">
                  + Agregar otro tipo de huevo
                </button>
              )}
            </div>
            ...
            <div className="bg-gray-50 rounded-xl p-3 space-y-1.5">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="text-blue-700">{fmt(totalCosto)}</span>
              </div>
              <div className="h-px bg-gray-200" />
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">Tu ganancia</span>
                <span className="font-semibold text-green-700">{fmt(ganancia)}</span>
              </div>
            </div>

            <button onClick={handleDespachar} className="w-full bg-green-500 hover:bg-green-600 active:scale-95 text-white font-semibold rounded-xl py-3 text-sm flex items-center justify-center gap-2 transition-all">
              <span>📤</span> Despachar y enviar por WhatsApp
            </button>

            {showSuccess && (
              <div className="text-center text-sm text-green-700 bg-green-50 rounded-xl py-2 border border-green-100">
                ✓ Despacho registrado y WhatsApp abierto
              </div>
            )}
          </>
        )}
...
function Cobros({ despachos, clients, onPago, onDelete }) {
  ...
  function handleAbono(d) {
    const abono = Number(abonoVal);
    if (!abono || abono <= 0) return;
    onPago(d.id, d.paid + abono);
    setAbonoVal(""); setSelectedId(null);
  }
    ...
    <div className="space-y-4">
      {pendientes.length === 0 && (
        <div className="text-center py-10 text-gray-400 text-sm">
          <p className="text-3xl mb-2">🎉</p>
          <p>Todo al día, sin cobros pendientes</p>
        </div>
      )}
        ...
        return (
          <div key={d.id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
            <div className="px-4 py-3">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">{resumenLineas(d)} · {formatDate(d.date)} · #{String(d.invoiceNum).padStart(4,"0")}</p>
                  {status === "parcial" && <p className="text-xs text-orange-600 mt-0.5">Abonó {fmt(d.paid)} · Resta {fmt(pendiente)}</p>}
                </div>
                <div className="text-right">
                  <p className="font-bold text-gray-800 text-sm">{fmt(pendiente)}</p>
            ...
            <div className="px-4 pb-3 flex gap-2">
              <DeleteButton onConfirm={() => onDelete(d.id)} />
              <button onClick={() => setSelectedId(isOpen ? null : d.id)} className="flex-1 border border-gray-200 text-gray-600 text-xs rounded-xl py-2 font-medium hover:bg-gray-50 active:scale-95 transition-all">
                {isOpen ? "Cancelar" : "Registrar abono"}
              </button>
              <button onClick={() => onPago(d.id, total)} className="flex-1 bg-green-500 text-white text-xs rounded-xl py-2 font-semibold hover:bg-green-600 active:scale-95 transition-all">
                Pago completo ✓
              </button>
            </div>
            {isOpen && (
              <div className="px-4 pb-4 flex gap-2 border-t border-gray-50 pt-3">
                ...
                <button onClick={() => handleAbono(d)} className="bg-amber-500 text-white text-xs rounded-xl px-4 font-semibold hover:bg-amber-600 active:scale-95 transition-all">Guardar</button>
              </div>
            )}
          </div>
        );
            ...
            return (
              <div key={d.id} className="bg-white rounded-2xl border border-gray-100 px-4 py-3 flex justify-between items-center opacity-70">
                <div>
                  ...
                  <p className="text-xs text-gray-400">{resumenLineas(d)} · {formatDate(d.date)}</p>
                </div>
                <div className="text-right">
                  <p className="font-semibold text-gray-700 text-sm">{fmt(totalDespacho(d))}</p>
                  <Badge status="pagado" />
                </div>
              </div>
            );
...
function Proveedor({ despachos, clients, onProviderPaid, onDelete }) {
  const pendientes = despachos.filter(d => !d.providerPaid).sort((a,b) => b.id - a.id);
  const pagados    = despachos.filter(d => d.providerPaid).sort((a,b) => b.id - a.id);
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-blue-50 rounded-2xl p-3 border border-blue-100">
          <p className="text-xs text-blue-700 font-medium mb-1">Le debo</p>
          <p className="text-xl font-bold text-blue-900">{fmt(totalDebo)}</p>
        </div>
        <div className="bg-green-50 rounded-2xl p-3 border border-green-100">
          <p className="text-xs text-green-700 font-medium mb-1">Ya le pagué</p>
          <p className="text-xl font-bold text-green-900">{fmt(totalPague)}</p>
        </div>
      </div>

      {pendientes.length === 0 && (
        <div className="text-center py-8 text-gray-400 text-sm">
          <p className="text-3xl mb-2">✅</p><p>Proveedor al día</p>
        </div>
      )}
          ...
          {pendientes.map(d => {
            ...
            return (
              <div key={d.id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <div className="flex gap-2 mt-1 flex-wrap">
                        {d.lineas.map(l => (
                          <span key={l.tipo} className="text-xs text-blue-700 bg-blue-50 border border-blue-100 px-2 py-0.5 rounded-full">
                            {l.tipo}: {fmt(l.cubetas * l.costPrice)}
                          </span>
                        ))}
                      </div>
                ...
                <div className="px-4 pb-3 flex gap-2">
                  <DeleteButton onConfirm={() => onDelete(d.id)} />
                  <button onClick={() => onProviderPaid(d.id)} className="flex-1 bg-blue-600 text-white text-xs rounded-xl py-2.5 font-semibold hover:bg-blue-700 active:scale-95 transition-all">
                    Marcar como pagado al proveedor ✓
                  </button>
                </div>
              </div>
            );
          ...
          {pagados.map(d => {
            ...
            return (
              <div key={d.id} className="bg-white rounded-2xl border border-gray-100 px-4 py-3 flex justify-between items-center opacity-60">
                ...
                <div className="text-right">
                  <p className="font-semibold text-gray-700 text-sm">{fmt(costoDespacho(d))}</p>
                  <span className="text-xs text-green-700 bg-green-50 border border-green-100 px-2 py-0.5 rounded-full">Pagado ✓</span>
                </div>
              </div>
            );
          })}
...
function Clientes({ clients, onAddClient, onDeleteClient }) {
  ...
  function handleAdd() {
    ...
    onAddClient({ name: name.trim(), phone: intl });
    setName(""); setPhone(""); setAdding(false);
  }

  return (
    <div className="space-y-3">
      <button onClick={() => setAdding(!adding)} className="w-full border-2 border-dashed border-yellow-300 text-yellow-700 rounded-2xl py-3 text-sm font-medium hover:bg-yellow-50 active:scale-95 transition-all">
        {adding ? "Cancelar" : "+ Agregar cliente"}
      </button>
      {adding && (
        <div className="bg-white rounded-2xl border border-gray-100 p-4 space-y-3">
          ...
          <button onClick={handleAdd} className="w-full bg-yellow-500 text-white text-sm rounded-xl py-2.5 font-semibold hover:bg-yellow-600 active:scale-95 transition-all">Guardar cliente</button>
        </div>
      )}
      {clients.length === 0 && !adding && (
        <div className="text-center py-8 text-gray-400 text-sm">
          <p className="text-3xl mb-2">👥</p><p>Aún no hay clientes</p>
        </div>
      )}
      {clients.map(c => (
...
function Precios({ prices, onSavePrices }) {
  ...
  function handleSave() {
    const parsed = {};
    EGG_TYPES.forEach(t => {
      parsed[t] = { sale: Number(vals[t].sale), cost: Number(vals[t].cost) };
    });
    onSavePrices(parsed);
    setSaved(true);
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-4 space-y-5">
        ...
        {EGG_TYPES.map(t => (
          <div key={t}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs text-gray-500 mb-1 block">Precio al cliente</label>
                <input type="number" className={inputCls} value={vals[t].sale} onChange={e => update(t, "sale", e.target.value)} />
              </div>
              <div>
                <label className="text-xs text-gray-500 mb-1 block">Costo proveedor</label>
                <input type="number" className={inputCls} value={vals[t].cost} onChange={e => update(t, "cost", e.target.value)} />
              </div>
            </div>
            <p className="text-xs text-green-700 mt-1.5">Margen: {fmt(Number(vals[t].sale) - Number(vals[t].cost))} por cubeta</p>
          </div>
        ))}
        <button onClick={handleSave} className="w-full bg-gray-800 text-white text-sm rounded-xl py-3 font-semibold hover:bg-gray-900 active:scale-95 transition-all">
          Guardar precios
        </button>
...
export default function App() {
  ...
  const [nextInvoice, setNextInvoice] = useState(1);
  const [nextId, setNextId] = useState(1);

  function addDespacho(data) {
    const newD = { id: nextId, invoiceNum: nextInvoice, date: new Date().toISOString().split("T")[0], paid: 0, providerPaid: false, ...data };
    setDespachos(prev => [...prev, newD]);
    setNextId(n => n + 1);
    setNextInvoice(n => n + 1);
    return newD;
  }

  function deleteDespacho(id) { setDespachos(prev => prev.filter(d => d.id !== id)); }
  function updatePago(id, newPaid) { setDespachos(prev => prev.map(d => d.id === id ? { ...d, paid: newPaid } : d)); }
  function updateProviderPaid(id) { setDespachos(prev => prev.map(d => d.id === id ? { ...d, providerPaid: true } : d)); }
  function addClient(data) { setClients(prev => [...prev, { id: Date.now(), ...data }]); }
  ...
  const providerCount = despachos.filter(d => !d.providerPaid).length;
    ...
    <div className="min-h-screen bg-gray-50" style={{ fontFamily: "'Inter', sans-serif", maxWidth: 430, margin: "0 auto" }}>
      <div className="bg-white border-b border-gray-100 px-4 pt-10 pb-4 sticky top-0 z-10">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-base font-bold text-gray-900 leading-tight">La Gallina Turuleca</h1>
      ...
      <div className="px-4 py-4 pb-28">
        {tab === "resumen"   && <Resumen   despachos={despachos} clients={clients} prices={prices} onDelete={deleteDespacho} />}
        {tab === "despachar" && <Despachar clients={clients} prices={prices} onDespacho={addDespacho} />}
        {tab === "cobros"    && <Cobros    despachos={despachos} clients={clients} onPago={updatePago} onDelete={deleteDespacho} />}
        {tab === "proveedor" && <Proveedor despachos={despachos} clients={clients} onProviderPaid={updateProviderPaid} onDelete={deleteDespacho} />}
        {tab === "clientes"  && <Clientes  clients={clients} onAddClient={addClient} onDeleteClient={deleteClient} />}
        {tab === "precios"   && <Precios   prices={prices} onSavePrices={setPrices} />}
      </div>
